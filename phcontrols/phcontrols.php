<?php
/*                       PHCONTROLS
 Librería que incluye rutinas de conexión a base de datos MySQL y la generación
 de controles y formularios HTML.
 La conexión con la base de datos permite agregar, editar o eliminar registros 
 de una tabla.
 Las tablas en MySQL deben complir con:
 - Las tablas a agregar o editar, deben tener siempre una clave primaria.
 - Los valores booleanos se representarán como valores Tinyint, porque las 
   rutinas internas asumen que todo campo Tinyint se manejará como booleno, al 
   momento de agregar o modificar tablas.
 - Los valores booleanos deben tener como valor por defecto el valor 0, para 
   evitar que se ingresen valores NULL.
 - Los campos de contraseña, se representarán como campos de tipo CHAR, para 
   que se muestren correctamente en los formularios.
 - Los campos que almacenen cadenas de varias líneas deben tener el tipo TEXT, 
   para que se representen como cuadro de texto de Varias líneas.

				Por Tito Hinostroza 2020 - Derechos Reservados
*/
function href_this_file() {
	/*Devuelve la URL de la carpeta de la librería actual (No de la página actual) 
	 incluyendo el '/' final. Se usa para referenciar a las imágenes de esta librería. 
	 Por ejemplo si la página actual es:  http://localhost/test/blocks.php y este
	 archivo PHP está ubicado en disco en: C:\xampp\htdocs (mapeado en http://localhost)
	 entonces, esta función devolverá: http://localhost/test/../
	 Este código está basado en uno de los ejemplos de 
	...
	$dir_pagactual = dirname(realpath($_SERVER['SCRIPT_FILENAME']));  //realpath() por si hay enlace simbólico.
	//echo '$dir_pagactual='.$dir_pagactual."\n"; //Sin "/" final
	//echo ' __DIR__='.__DIR__."\n";	//Sin "/" final
	$arFrom = explode(DIRECTORY_SEPARATOR, $dir_pagactual);
	$arTo = explode(DIRECTORY_SEPARATOR, __DIR__);
	while(count($arFrom) && count($arTo) && ($arFrom[0] == $arTo[0])) {
		//Elimina elemento común
		array_shift($arFrom);	
		array_shift($arTo);
	}
	//Parte de la ruta del URL de la página actual
	$H_THIS_DIR = '//'.dirname($_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']).'/';
	//echo $H_THIS_DIR."\n";
	//Completa con dirección de retroceso: ../../
	$H_THIS_DIR .= str_pad("", count($arFrom) * 3, '../');
	//Completa con información de carpetas de avance: /<dir1>/<dir2>
	$H_THIS_DIR .= implode('/', $arTo);
	return count($arTo)>0 ? $H_THIS_DIR.'/' : $H_THIS_DIR;
}	
////////// Íconos usados
	$path_icons = href_this_file();	//Podría omitirse si no es necesario para evitar carga adicional.
	$ICO_TRASH  = '<img src="'.$path_icons.'bin.png" alt="trash icon">';
	$ICO_UPDATE = '<img src="'.$path_icons.'spinner11.png" alt="update icon">';
